...
def load_filtered_data():
    filtered_designs=pd.read_csv('datasets/rd6_metrics_fig4b-g.csv')
    scores=pd.read_csv('datasets/rd6_stability_scores_fig4b-g.csv')
    merged=pd.merge(left=filtered_designs, right=scores[['name', 'stabilityscore']], on='name', how='inner')
    return merged
filtered_designs =load_filtered_data()



def filter_abegos(df):
    ...
    # get abego and strands to analyze
    n_loops = 3
    abegos_to_analyze = []
    for i in range(n_loops):
    ...
    return feat_to_analyze, filter_abego_names, filter_strands
feat_to_analyze, filter_abego_names, filter_strands = filter_abegos(df=filtered_designs)



def get_dummies(df):
    dummy_df = pd.get_dummies(df, columns = ['L1', 'L2', 'L3', 'len_strand1'])
    return pd.concat([df[['L1', 'L2', 'L3', 'len_strand1']], dummy_df], axis = 1)
filtered_designs = get_dummies(df=filtered_designs)


def regression_model_loops(df, n_iterations):
...
summary_df, feat_to_analyze, = regression_model_loops(df=filtered_designs, n_iterations=1000)
...
def keep_designs_abego_combinations(df):
    ...
    return df2
sub_df = keep_designs_abego_combinations(df=filtered_designs)
...
def success_rate_btsp(n_iterations):
    ''' determine success ratio by bootstrapping '''
